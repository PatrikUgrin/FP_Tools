...
export enum FONT_SOURCE{
	webfont
}


//https://github.com/mimshwright/pixi-tagged-text @ts-ignore
export class FPText extends TaggedText{
	public type : FONT_SOURCE;
	public translation_key : string;

	private static readonly DEFAULT_TAG_STYLES = {
		default: {
	...
	constructor(
		text : string,
		use_translation_key : boolean = true,
		tagStyles? : Partial<typeof FPText.DEFAULT_TAG_STYLES>,
	) {
		let translation_key : string | undefined;
		if(use_translation_key){
			if(Translations.getInstance().isLoaded()){
				translation_key = text;
				text = Translations.getInstance().get(text);
			}else{
				fpglobals.GLog("Translations: not yet initialised — call init() first.", log.type.ERROR);
			}
		}
		super(text, { ...FPText.DEFAULT_TAG_STYLES, ...tagStyles });
		if(translation_key !== undefined){
			this.translation_key = translation_key;
		}
	}

	public updateTextTranslation(text : string = this.translation_key, use_translation_key : boolean = true){
		if(use_translation_key){
			if(Translations.getInstance().isLoaded()){
				this.translation_key = text;
				this.text = Translations.getInstance().get(text);
			}else{
				fpglobals.GLog("Translations: not yet initialised — call init() first.", log.type.ERROR);
			}
		}
...
//Documentation for pixi-tagged-text:
...
Tags
    ...
    Tags that do not have a matching style are NOT treated as tags. <i><bold>just italic!</bold></i> would render as <bold>just italic!</bold> if there were a style for i but not bold.
...
(By the way, there is an excellent TextStyle visual editor that you can use to preview how your style will appear. The style objects generated can be used in this component.)
    ...
    fontSize - Has the added ability to use percentage-based sizes which are based on the fontSize in the parent tag. In the example: <small fontSize="10">small<big fontSize="300%">big</big></small>, "small" will be 10px and "big" will be 30px. The default fontSize is 26px.
...
The third parameter in the TaggedText constructor is a set of options.
    ...
    overdrawDecorations - When using text decorations (e.g. underline) an additional length in pixels equal to overdrawDecorations will be added to both sides of the line. Default is 0.
...
These properties are available:
    ...
    spriteTemplates - The sprites in sprites and spriteContainer are actually clones of the originals passed in via the imgMap option. To get the originals, access them this way.
...
In order to maximize performance of TaggedText, it helps to understand how it renders the input.
    ...
    Constructor - creating the component is the first step. You can set text, styles, and options in the constructor.
    update() - Update generates a list of plain JS object tokens that hold information on what type of text to create and where to position it. The tokens contain all the information you need to draw the text and are saved as the instance member tokens but also returned by the update() method. Aside from decoupling from the render code, this allows us to write tests to verify every step of the lexing, styling and layout independently without drawing anything at all. By default, this is called every time the text or style definitions are changed (e.g. setTagStyles(), setText()). This is a fairly expensive process but usually faster than draw().
    draw() - Draw creates the child objects based on the data generated by update(). It clears any existing children (if needed) then recreates and positions them. This is probably the costliest method in the life-cycle. By default, this is called automatically by update().
    Of course, you won't see anything on your screen until your component is added to a visible PIXI container that's part of the stage in a pixi app.
